...
private:
    ...
    std::map<const char, uint32_t> flags {
        {'r', 0x01},    // 「全角」英字を「半角」に変換します。
        {'R', 0x10},   // 「半角」英字を「全角」に変換します。
        {'n', 0x02},    // 「全角」数字を「半角」に変換します。
        {'N', 0x20},   // 「半角」数字を「全角」に変換します。
        {'a', 0x03},    // 「全角」英数字を「半角」に変換します。0x1  | 0x2
        {'A', 0x30},   // 「半角」英数字を「全角」に変換します。0x10 | 0x20
                        // （"a", "A" オプションに含まれる文字は、U+0022, U+0027, U+005C, U+007Eを除く U+0021 - U+007E の範囲です）。
        {'s', 0x08},   // 「全角」スペースを「半角」に変換します（U+3000 -> U+0020）。
        {'S', 0x80},  // 「半角」スペースを「全角」に変換します（U+0020 -> U+3000）。

        {'k', 0x0100},   // 「全角カタカナ」を「半角カタカナ」に変換します。
        {'K', 0x1000},  // 「半角カタカナ」を「全角カタカナ」に変換します。
        {'h', 0x0200},   // 「全角ひらがな」を「半角カタカナ」に変換します。
        {'H', 0x2000},  // 「半角カタカナ」を「全角ひらがな」に変換します。
        {'c', 0x0400},  // 「全角カタカナ」を「全角ひらがな」に変換します。
        {'C', 0x4000}, // 「全角ひらがな」を「全角カタカナ」に変換します。
        //{'m', 0x0800},  // 「全角」記号を「半角」に変換します。
        //{'M', 0x8000},  // 「半角」記号を「全角」に変換します。
        {'V', 0xf000}   // 濁点付きの文字を一文字に変換します。"K", "H" と共に使用します。
    };
    int opts = 0;
    bool Parse() {
...
static const std::vector<std::vector<std::string>> KIGO_MAP = {
    {{}},
};
